...
FROM xychelsea/anaconda3:v23.1.0
LABEL description="text-generation-webui Vanilla Container"

# $ docker build --network=host -t xychelsea/text-generation-webui:latest -f Dockerfile .
# $ docker run --rm -it -d -p 7860:7860 --rm -it xychelsea/text-generation-webui:latest /bin/bash
# $ docker push xychelsea/text-generation-webui:latest
...
COPY environment.yml ${PROJECT_PATH}/environment.yml
